...
const ConfigLayout = () => {
  ...
  const handleAddNewCamera = (newCamera) => {
    axios
      .post(`${SERVER_URL}/api/camera/`, newCamera, {
        headers: {
          "Content-Type": "application/json",
          "x-access-token": user.token,
        },
  ...
  const handleUpdateCamera = (updatedCamera) => {
    axios
      .put(
        `${SERVER_URL}/api/camera/${cameras[selectedCameraIndex].id}`,
        updatedCamera,
        {
          headers: {
            "Content-Type": "application/json",
          ...
          <Grid item xs={12}>
            <Paper sx={{ height: "43vh", border: "1px solid #d3d3d3" }}>
              <MapWithNoSSR
                lat={
                  selectedCameraIndex
                    ? cameras[selectedCameraIndex].location.coordinates[1]
                    : null
                }
                lng={
                  selectedCameraIndex
                    ? cameras[selectedCameraIndex].location.coordinates[0]
                    : null
                }
                selected={selectedCameraIndex}
              />
              ...
              {isEditing ? (
                ...
                >
                  <h3>
                    ...
                    Stream of{" "}
                    {selectedCameraIndex !== null
                      ? cameras[selectedCameraIndex].camera_name
                      : "camera"}{" "}
                  </h3>
                </Box>
              )}
