...
export const helpIcon = new Icon({
  iconUrl: "/images/help.svg",
  iconSize: [40, 40],
});
export const cctvIcon = new Icon({
  iconUrl: "/images/cctv.svg",
  iconSize: [40, 40],
});
...
const HomeMap = ({
  ...
  useEffect(() => {
    if (requests[selectedRequestIndex]) {
      setCenter([
        requests[selectedRequestIndex].location.lat,
        requests[selectedRequestIndex].location.lng,
      ]);
    }
      ...
      {requests &&
        requests.map((request, index) => (
          <Marker
            ...
            icon={helpIcon}
            eventHandlers={{ click: () => handleRequestClick(index) }}
          />
        ))}
      {cameras &&
